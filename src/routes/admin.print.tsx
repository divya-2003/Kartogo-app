...
export const Route = createFileRoute("/admin/print")({ component: PrintQueueAdmin });
...
function PrintQueueAdmin() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try { setJobs(await listPrintJobsFn({ data: { adminToken } })); }
  ...
  const download = async (job: PrintJob) => {
    if (!adminToken) return;
    try {
      const f = await getPrintJobFileFn({ data: { adminToken, id: job.id } });
  ...
  const setStatus = async (job: PrintJob, status: PrintStatus) => {
    if (!adminToken) return;
    try {
      const updated = await setPrintJobStatusFn({ data: { adminToken, id: job.id, status } });
